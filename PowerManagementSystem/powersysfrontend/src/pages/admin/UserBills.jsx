...
const UserBills = () => {
    ...
    const editBill = (event, param) => {
        navigate('/admin/edit-bill', {
            state: { billobj: param, us_name: us_name },
        })
    }

    const deleteBill = (event, param) => {
        deletBillService(param.bill_Id)
            .then((data) => {
                getUserDetails(us_id)
                    .then((data) => {
                        let usr = data
    ...
    return (
        <Base>
            <Container style={{ marginTop: '20px' }}>
                <Row>
                    <Col>
                        <h3>User's Bills: (Name: {us_name})</h3>
                    </Col>
                    <Col xs={2}>
                        <Button
                            outline
                            className="text-right"
                            onClick={(event) => addNewBill(event)}
                        >
                            Add New Bill
                        </Button>
                    </Col>
